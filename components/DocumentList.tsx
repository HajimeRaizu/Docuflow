...
export const DocumentList: React.FC = () => {
  ...
  useEffect(() => {
    const loadDocs = () => {
        try {
            const stored = localStorage.getItem('nemsu_smartdraft_docs');
            if (stored) {
                const parsed = JSON.parse(stored);
                // Convert string dates back to Date objects if needed, 
                // though usually strings are fine for simple display
                setDocs(parsed);
            }
        } catch(e) {
    ...
    loadDocs();
  }, [historyDoc]); // Reload when closing history modal to ensure updates if any

  const handleDelete = (id: string) => {
    if (confirm("Are you sure you want to delete this document?")) {
        const updated = docs.filter(d => d.id !== id);
        setDocs(updated);
        localStorage.setItem('nemsu_smartdraft_docs', JSON.stringify(updated));
    }
  };
